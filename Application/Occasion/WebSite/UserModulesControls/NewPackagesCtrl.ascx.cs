...
public partial class NewPackagesCtrl : BaseControl
{
    ...
    {
        List<Offer> offersList = offersOperator.SelectNewPackages(IsArabic);

        if (offersList != null && offersList.Count > 0)
        {
            rptNewPackages.DataSource = offersList;
            rptNewPackages.DataBind();
        }
    }
