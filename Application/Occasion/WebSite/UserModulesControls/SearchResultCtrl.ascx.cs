...
public partial class SearchResultCtrl : BaseControl
{
    ...
    private void SearchOffers()
    {
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["q"]))
            searchWord = HttpUtility.UrlDecode(Request.QueryString["q"].Trim());

        string catID = null;
        if (!string.IsNullOrEmpty(Request.QueryString["CatID"]))
            catID = Request.QueryString["CatID"].Trim();

        if (!string.IsNullOrEmpty(searchWord) && string.IsNullOrEmpty(catID))
        {
            MainPager.DataSource = offersOperator.Search(searchWord, Utility.HasArabicLetters(searchWord), IsArabic);
        }
        else if (string.IsNullOrEmpty(searchWord) && !string.IsNullOrEmpty(catID))
        {
            ArrayList categoryList = new ArrayList();
            GetCategoriesList(Convert.ToInt32(catID), categoryList);
        ...
        else if (!string.IsNullOrEmpty(searchWord) && !string.IsNullOrEmpty(catID))
        {
            ...
            if (categoryList != null && categoryList.Count > 0)
                MainPager.DataSource = offersOperator.Search(searchWord, categoryList, Utility.HasArabicLetters(searchWord), IsArabic);
            else
                MainPager.DataSource = null;
        }
        else if (string.IsNullOrEmpty(searchWord) && string.IsNullOrEmpty(catID))
        {
            MainPager.DataSource = offersOperator.SelectAll("All", IsArabic, true);
        }
    ...
    private void BindMainRepeater()
    {
        if (MainPager.DataSource != null && MainPager.DataSourceCount > 0 && drpFilterBy.SelectedItem.Value == "None")
        {
            ...
            if (MainPager.PageCount > 1)
            {
                ...
                ltrlCount.Text = string.Concat(Literals.SearchResults, ' ', Convert.ToString(startItem), ' ', Literals.To, ' ', Convert.ToString(lastItem), ' ', Literals.From, ' ', Convert.ToString(MainPager.DataSourceCount), ' ', Literals.Items);
            }
            else
            {
                if (MainPager.DataSourceCount > 1)
                    ltrlCount.Text = string.Concat(Literals.SearchResults, ' ', Literals.Is, ' ', Convert.ToString(MainPager.DataSourceCount), ' ', Literals.Items);
                else
                    ltrlCount.Text = string.Concat(Literals.SearchResults, ' ', Literals.Is, ' ', Convert.ToString(MainPager.DataSourceCount), ' ', Literals.Item);
    ...
    private PagedDataSource MainPager
    {
        get
        {
            if (Session["SearchMainPager"] != null)
                return (PagedDataSource)Session["SearchMainPager"];
            else
                return null;
        }
        set
        {
            Session["SearchMainPager"] = value;
        }
    }
    ...
    private List<KeyValue> SubPagerList
    {
        get
        {
            if (Session["SearchSubPager"] != null)
                return (List<KeyValue>)Session["SearchSubPager"];
            else
                return null;
        }
        set
        {
            Session["SearchSubPager"] = value;
        }
    }
