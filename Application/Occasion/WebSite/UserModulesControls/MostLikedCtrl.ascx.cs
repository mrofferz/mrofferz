...
public partial class MostLikedCtrl : BaseControl
{
    ...
    {
        List<Offer> offersList = offersOperator.SelectMostLiked("All", IsArabic);

        if (offersList != null && offersList.Count > 0)
        {
            rptMostLiked.DataSource = offersList;
            rptMostLiked.DataBind();
        }
    }
