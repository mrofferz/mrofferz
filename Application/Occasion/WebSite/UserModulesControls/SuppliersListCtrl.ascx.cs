...
public partial class SuppliersListCtrl : BaseControl
{
    ...
    private int PageCount
    {
        get
        {
            if (Session["SuppliersListPageSize"] != null)
                return Convert.ToInt32(Session["SuppliersListPageSize"]);

            return 0;
        }
        set
        {
            Session["SuppliersListPageSize"] = value;
        }
    }
    ...
    protected void rptSuppliers_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        try
        {
            if (e.CommandName == "ViewOffers")
                redirectPage = Utility.AppendQueryString(PagesPathes.OffersList, new KeyValue("SuppID", Convert.ToString(e.CommandArgument)));

            else if (e.CommandName == "ViewDetails")
                redirectPage = Utility.AppendQueryString(PagesPathes.ViewSupplierDetails, new KeyValue(CommonStrings.ID, Convert.ToString(e.CommandArgument)));
        }
        catch
        {
    ...
    protected override void LoadPagedData()
    {
        ...
        if (suppliersList != null && suppliersList.Count > 0)
        {
            ...
            pager.DataSource = suppliersList;
