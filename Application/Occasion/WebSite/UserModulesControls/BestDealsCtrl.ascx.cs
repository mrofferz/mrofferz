...
public partial class BestDealsCtrl : BaseControl
{
    ...
    {
        List<Offer> offersList = offersOperator.SelectBestDeals("All", IsArabic);

        if (offersList != null && offersList.Count > 0)
        {
            rptBestDeals.DataSource = offersList;
            rptBestDeals.DataBind();
        }
    }
