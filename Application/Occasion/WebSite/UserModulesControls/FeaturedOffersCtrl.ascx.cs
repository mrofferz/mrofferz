...
public partial class FeaturedOffersCtrl : BaseControl
{
    ...
    {
        List<Offer> offersList = offersOperator.SelectFeatured("All", IsArabic);

        if (offersList != null && offersList.Count > 0)
        {
            rptFeaturedOffersLarge.DataSource = offersList;
            rptFeaturedOffersLarge.DataBind();

            rptFeaturedOffersSmall.DataSource = offersList;
            rptFeaturedOffersSmall.DataBind();
        }
    }
