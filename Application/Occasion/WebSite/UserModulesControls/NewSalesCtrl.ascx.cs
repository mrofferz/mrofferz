...
public partial class NewSalesCtrl : BaseControl
{
    ...
    {
        List<Offer> offersList = offersOperator.SelectNewSales(IsArabic);

        if (offersList != null && offersList.Count > 0)
        {
            rptNewSales.DataSource = offersList;
            rptNewSales.DataBind();
        }
    }
