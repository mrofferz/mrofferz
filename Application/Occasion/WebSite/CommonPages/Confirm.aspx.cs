...
public partial class Confirm : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Request.QueryString[CommonStrings.Message] != null)
            {
                lblMessage.Text =
                    Notifications.ResourceManager.GetString(Request.QueryString[CommonStrings.Message]);
            }
            else
            {
                lblMessage.Text = Notifications.GeneralMessage;
            }
